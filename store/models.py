...
from store.utils import path_and_rename, group_image, category_image

NO_IMAGE_URL = '/static/img/no_image.png'
NO_IMAGE_THUMB = '/static/img/no_image_thumb.png'
NO_IMAGE_GROUP = '/static/img/no_image_group.png'
...
class Group(models.Model):
    ...
    class Meta:
        verbose_name = 'Группа'
        verbose_name_plural = '6. Группы'
        ordering = ('id',)

    id = models.AutoField(primary_key=True)
    name = models.CharField(max_length=255, verbose_name='Группа')
    image = models.ImageField(verbose_name='Изображение', blank=True,
                              null=True, upload_to=group_image,
                              help_text='Изображение размером 350x130 пикселей')
    start_page = models.BooleanField(verbose_name='Разместить на стартовой', default=False,
    ...
    def image_thumb(self):
        img_src = f"/media/{self.image}" if self.image else NO_IMAGE_GROUP
        return mark_safe(
            f'<img src="{img_src}" height="50" />'
        )

    def image_name(self):
        return f"/media/{self.image}" if self.image else NO_IMAGE_GROUP
...
class Category(models.Model):
    ...
    image = models.ImageField(verbose_name='Изображение', blank=True,
                              null=True, upload_to=category_image,
                              help_text='Изображение размером 350x130 пикселей')
...
class Customer(models.Model):
    class Meta:
        ...
        verbose_name_plural = '2. Покупатели'
...
class Article(models.Model):
    class Meta:
        ...
        verbose_name_plural = '4. Страницы'
        ordering = ('id',)
